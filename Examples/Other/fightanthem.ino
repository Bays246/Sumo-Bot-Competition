...
bool displayCleared;
const char fugue[] PROGMEM =                                             /////////////////////////////////song here//////////////////////////////////////////////
"O3 <c#8 <c#16 <b16 d8 d16 <b16 d#8 d#16 <b16 e8 e16 <b16";
...
void loop()
{
  buzzer.playFromProgramSpace(fugue);                                              ///////////////////////////This starts the song when the robot is turned on////////////////////////////////////
  while(buzzer.isPlaying()){ 
  bool buttonPressA = buttonA.getSingleDebouncedPress();                           //////////////////////////Music will constantly loop till turned off///////////////////////////////////////////
  ...
  if (state == StatePausing)
  {
    ...
    if (buttonPressA)
    {
      // The user pressed button A, so go to the waiting state.                   ///////////////////////////////Add code here if you want to have music play when button is pressed I cant rember if it loops this way or not////////////////////////
      changeState(StateWaitingA);                                                 
    }
    if (buttonPressB)
    {
      // The user pressed button A, so go to the waiting state.
      changeState(StateWaitingB);
    }
  }
  ...
  else if (state == StateWaitingA)
  {
    ...
    else
    {
      ...
      changeState(StateStartA);
    }
  }
  else if (state == StateWaitingB)
  {
    // In this state, we wait for a while and then move on to the
    // scanning state.
    ...
    {
      // We have waited long enough.  Start moving.
      changeState(StateStartB);
    }
  }
  else if (state == StateStartA)
  {
    uint16_t time = timeInThisState();
